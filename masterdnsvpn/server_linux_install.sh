...
echo -e "        MasterDnsVPN Server Auto-Installer (Rust)${NC}"
echo -e "${CYAN}------------------------------------------------------${NC}"
...
if [[ "$ARCH" == "aarch64" || "$ARCH" == "arm64" ]]; then
  URL="https://github.com/masterking32/MasterDnsVPN/releases/latest/download/MasterDnsVPN_Server_Rust_Linux_ARM64.zip"
  PREFIX="MasterDnsVPN_Server_Rust_Linux_ARM64"
elif [[ "$ARCH" == "x86_64" ]]; then
  URL="https://github.com/masterking32/MasterDnsVPN/releases/latest/download/MasterDnsVPN_Server_Rust_Linux_AMD64.zip"
  PREFIX="MasterDnsVPN_Server_Rust_Linux_AMD64"
else
...
EXECUTABLE=""
# Look for the Rust server binary (masterdnsvpn-server)
if [[ -f "masterdnsvpn-server" ]]; then
  EXECUTABLE="masterdnsvpn-server"
else
  # Fallback: find any matching binary in extracted files
  EXECUTABLE="$(ls -t masterdnsvpn-server* 2>/dev/null | head -n1 || true)"
fi
[[ -z "$EXECUTABLE" ]] && log_error "Server binary not found in package."
chmod +x "$EXECUTABLE"

log_header "Configuration"
if [[ ! -f "server_config.toml" ]]; then
  if [[ -f "server_config.toml.simple" ]]; then
    cp server_config.toml.simple server_config.toml
    log_info "Created server_config.toml from sample template."
  else
    log_error "server_config.toml not found after extraction."
  fi
fi

CURRENT_VERSION="$(extract_config_version server_config.toml)"
if [[ -z "${CURRENT_VERSION:-}" ]]; then
  log_warn "CONFIG_VERSION not found in server_config.toml. Proceeding anyway."
fi
if [[ -f "server_config.toml.backup" ]]; then
  BACKUP_VERSION="$(extract_config_version server_config.toml.backup)"
  if [[ -z "${BACKUP_VERSION:-}" ]]; then
    log_warn "Backup config has no CONFIG_VERSION. Using fresh config."
    ...
    if [[ "$BACKUP_VERSION" == "$CURRENT_VERSION" ]]; then
      ...
      log_warn "Backup config version ($BACKUP_VERSION) >= package config ($CURRENT_VERSION). Using backup."
      mv -f server_config.toml.backup server_config.toml
    fi
  fi
fi
...
cat > "$SVC" <<EOF
[Unit]
Description=MasterDnsVPN Server (Rust)
